document.addEventListener("DOMContentLoaded", () => {
    ...
    const offsets = {
        ...
        seentyFivePercentCheckpoint: { percentage: "75%", message: "75% to 100% read!", location: articleLength * 0.75 + getTheArticle().offsetTop },
    ...
    function checkpointEvents(percentage) {
        return new CustomEvent("checkpoint", {detail: {percentage: percentage}});
    };

    function dispatchScrollOffset(offset) {
        window.dispatchEvent(checkpointEvents(offset.percentage)); // <- this is really it
        alert(offset.message + " Past the " + offset.location + "px checkpoint."); // <- to let the user know
        console.log("Dispatched: ", checkpointEvents(offset.percentage)); // <- to show what was dispatched
    };
